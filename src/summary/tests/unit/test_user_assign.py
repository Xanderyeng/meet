...
class TestBuildSpeakerTimelines:
    ...
    def test_short_words_only_uses_segment_start_and_last_word_end(self):
        """With no overly long words, the interval runs segment.start to last word end."""
        transcription = FakeTranscription(
            segments=[
                {
                    "start": 1.0,
    ...
    def test_long_word_caps_interval_at_max_duration(self):
        """A word longer than MAX_WORD_DURATION truncates and ends the segment."""
        transcription = FakeTranscription(
            segments=[
                {
                    "start": 0.0,
